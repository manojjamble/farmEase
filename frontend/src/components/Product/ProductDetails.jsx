...
const ProductDetails = ({ machineId }) => {
    ...
    useEffect(() => {
        const fetchProductDetails = async () => {
            try {
                const response = await axios.get(`${BASE_URL}/api/machine?machineId=${machineId}`);
    ...
    const { name, company, category, description, availability, rentalPrice, ownerId, img, reviews } = machine;
    const remainingImages = img.slice().filter(image => image !== selectedImage);


    return (
        <div className="container mx-auto p-8 bg-gray-300 m-5 rounded-xl shadow-lg shadow-zinc-500">
            <h2 className="text-5xl text-green-900 text-center font-bold mb-4">{name}</h2>
            <div className="grid grid-cols-1 mt-9 lg:grid-cols-2 gap-8">
                ...
                <div className="flex flex-col ">
                    <p className="text-zinc-600 text-lg mb-4"><b>{description}</b></p>
                    ...
                    <div className="flex flex-col mb-4">
                        <span className="font-semibold text-2xl text-zinc-600 mb-2">Owner Details:</span>
                        <span><b>Name:</b> {ownerId.name}</span>
                        <span><b>Mobile:</b> {ownerId.mobile}</span>
                        <span><b>Address:</b> {ownerId.address}</span>
                        <span><b>City: </b>{ownerId.city}</span>
